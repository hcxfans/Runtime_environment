...
cdcodeinfo_dict,newlines,featureLines=save_data('四川',scfy_dict,xdhy_dict,cdfy_dict,scfy_dict,xdfy_dict)
cdcodeinfo_dict=dict(sorted(cdcodeinfo_dict.items(), key=lambda x: x[0],reverse=False))
with open(path5+'\\'+'mysccodeinfofile.json', 'w', encoding ='utf8') as json_file:
    json.dump(cdcodeinfo_dict, json_file, ensure_ascii = False,indent = 6)
with open(path4+'\\'+'四川方言词汇义类编码文本.csv',"w",encoding="utf-8") as tf:
    tf.writelines(newlines)
with open(path4+'\\'+'四川方言词汇特征词文本.csv',"w",encoding="utf-8") as tf:
            tf.writelines(featureLines)

sccode_dict,sccodestr_dict=save_cdcodedata(scfy_dict,xdhy_dict)
sccode_dict=dict(sorted(sccode_dict.items(), key=lambda x: x[0],reverse=False))
sccodestr_dict=dict(sorted(sccodestr_dict.items(), key=lambda x: x[0],reverse=False))
code_count_dict=fycd_cdfl_count(code_count_dict,sccode_dict)
with open(path5+'\\'+'sc_code_count_dict.json', 'w', encoding ='utf8') as json_file:
            json.dump(code_count_dict, json_file, ensure_ascii = False,indent = 6)
with open(path5+'\\'+'mysccodefile.json', 'w', encoding ='utf8') as json_file:
            json.dump(sccode_dict, json_file, ensure_ascii = False,indent = 6)
with open(path5+'\\'+'mysccodestrfile.json', 'w', encoding ='utf8') as json_file:
            json.dump(sccodestr_dict, json_file, ensure_ascii = False,indent = 6)
newlines=get_code_word_csv(sccode_dict)
with open(path4+'\\'+'四川方言分类词典.csv',"w",encoding="utf-8") as tf:
            tf.writelines(newlines)
with open(path2+'\\'+'四川方言分类词典.csv',"w",encoding="utf-8") as tf:
            tf.writelines(newlines)
newlines=dict_to_doc(path8+'\\header.txt',sccodestr_dict)
with open(path4+'\\'+'四川方言分类词典文本.txt',"w",encoding="utf-8") as tf:
            tf.writelines(newlines)
