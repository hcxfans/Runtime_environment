...
def split_word(word):#处理词语带括号及括号里有顿号情况
    ...
    #print(wordList)    
    return wordList
def split_cd_onekuo_word(word):#处理词语带括号及括号里有顿号情况
    wordList=[]
    
    tmpword=word.strip().rstrip()
    word1=substring_before(tmpword, '(')
    wordother=substring_after(tmpword, '(')
    if wordother:
        tmpword=substring_after(wordother, ')')
        wordother=substring_before(wordother, ')')
        
    wordList.append(''+word1+tmpword)
    wordList.append(''+word1+wordother+tmpword)
    return wordList

def split_cd_twokuo_word(word):#处理词语带括号及括号里有顿号情况
    wordList=[]
    word1=''
    word2=''
    wordother1=''
    ...
    tmpword=word.strip().rstrip()
    word1=substring_before(tmpword, '(')
    wordother1=substring_after(tmpword, '(')
    if wordother1:
        tmpword=substring_after(wordother1, ')')
        wordother1=substring_before(wordother1, ')')
        
    
    word2=substring_before(tmpword, '(')
    wordother2=substring_after(tmpword, '(')
    if wordother2:
        tmpword=substring_after(wordother2, ')')
        wordother2=substring_before(wordother2, ')')
        
    wordList.append(''+word1+word2+tmpword)
    wordList.append(''+word1+wordother1+word2+tmpword)
    wordList.append(''+word1+word2+wordother2+tmpword)
    wordList.append(''+word1+wordother1+word2+wordother2+tmpword)
    return wordList
def split_cd_word(word):#处理词语带括号及括号里有顿号情况
    kcount=get_kuocount(word)
    if kcount==0:
        return [word]
    elif kcount==1:
        return split_cd_onekuo_word(word)
    else:
        print(word)
        return split_cd_twokuo_word(word)
            ...
            for line in f.readlines():
                if line:
                    ...
                    if len(wordinfoList)>1:
                        ...
                        word_scList=split_cd_word(word_sc)
                        #print(word_scList)
                        #word_scList=[word_sc]
                        if word_scList:
...
def save_data(zone,fy_dict,xdhy_dict,cdfy_dict,scfy_dict,xdfy_dict):
    newlines=[]
    featureLines=[]
    cdcodeinfo_dict={}
    newlines.append('zone'+'\t'+'word'+'\t'+'code'+'\t'+'Sub_class1'+'\t'+'Sub_class2'+'\t'+'Sub_class3'+'\t'+'Origin'+'\n')
    featureLines.append('zone'+'\t'+'word'+'\t'+'code'+'\t'+'Sub_class1'+'\t'+'Sub_class2'+'\t'+'Sub_class3'+'\t'+'Origin'+'\n')
    for key in fy_dict.keys():
        #sub_1_clsinfo=tmp_dict.get(key1).get('一级分类')
        cdcode=getcdwordcodeinfo(key,fy_dict,xdhy_dict)
        if not cdcodeinfo_dict.get(cdcode[0]):
            cdcodeinfo_dict.update({cdcode[0]:cdcode[1]})
        codeinfoList=cdcode[1].split('->')
        if key==fy_dict[key] and key in xdfy_dict.keys():
            origin='大陆'
        elif key in scfy_dict.keys():
            origin='四川'
        else:
            origin='成都'
            featureLines.append(zone+'\t'+key+'\t'+cdcode[0]+'\t'+codeinfoList[0]+'\t'+codeinfoList[1]+'\t'+codeinfoList[2]+'\t'+origin+'\n')
        newlines.append(zone+'\t'+key+'\t'+cdcode[0]+'\t'+codeinfoList[0]+'\t'+codeinfoList[1]+'\t'+codeinfoList[2]+'\t'+origin+'\n')
        
    return cdcodeinfo_dict,newlines,featureLines
...
cdfy_dict={}
with open(path5+'\\'+'mycdfile.json', encoding="utf-8") as f:
    cdfy_dict= json.load(f)
xdfy_dict={}
with open(path5+'\\'+'myxdfile.json', encoding="utf-8") as f:
    xdfy_dict= json.load(f)
scfy_dict={}
with open(path5+'\\'+'myscfile.json', encoding="utf-8") as f:
    scfy_dict= json.load(f)
code_count_dict=init_cdfl_count(path8+'\\header.txt')
cdcodeinfo_dict,newlines,featureLines=save_data('成都',cdfy_dict,xdhy_dict,cdfy_dict,scfy_dict,xdfy_dict)
cdcodeinfo_dict=dict(sorted(cdcodeinfo_dict.items(), key=lambda x: x[0],reverse=False))
with open(path5+'\\'+'mycdcodeinfofile.json', 'w', encoding ='utf8') as json_file:
    json.dump(cdcodeinfo_dict, json_file, ensure_ascii = False,indent = 6)
with open(path4+'\\'+'成都方言词汇义类编码文本.csv',"w",encoding="utf-8") as tf:
            tf.writelines(newlines)
with open(path4+'\\'+'成都方言词汇特征词文本.csv',"w",encoding="utf-8") as tf:
            tf.writelines(featureLines)

cdcode_dict,cdcodestr_dict=save_cdcodedata(cdfy_dict,xdhy_dict)
cdcode_dict=dict(sorted(cdcode_dict.items(), key=lambda x: x[0],reverse=False))
cdcodestr_dict=dict(sorted(cdcodestr_dict.items(), key=lambda x: x[0],reverse=False))

code_count_dict=fycd_cdfl_count(code_count_dict,cdcode_dict)
with open(path5+'\\'+'cd_code_count_dict.json', 'w', encoding ='utf8') as json_file:
            json.dump(code_count_dict, json_file, ensure_ascii = False,indent = 6)

with open(path5+'\\'+'mycdcodefile.json', 'w', encoding ='utf8') as json_file:
            json.dump(cdcode_dict, json_file, ensure_ascii = False,indent = 6)
with open(path5+'\\'+'mycdcodestrfile.json', 'w', encoding ='utf8') as json_file:
            json.dump(cdcodestr_dict, json_file, ensure_ascii = False,indent = 6)
newlines=get_code_word_csv(cdcode_dict)
with open(path4+'\\'+'成都方言分类词典.csv',"w",encoding="utf-8") as tf:
            tf.writelines(newlines)
with open(path2+'\\'+'成都方言分类词典.csv',"w",encoding="utf-8") as tf:
            tf.writelines(newlines)
newlines=dict_to_doc(path8+'\\header.txt',cdcodestr_dict)
with open(path4+'\\'+'成都方言分类词典文本.txt',"w",encoding="utf-8") as tf:
            tf.writelines(newlines)
#cdfyinfo=getcdwordcodeinfo('𨃅',cdfy_dict,xdhy_dict)
#print(getcdwordcodeinfo('𨃅',cdfy_dict,xdhy_dict))


print('总共有'+str(len(wordlist))+'个词语等待测试！')
print('现在测试每个词语的语义分类情况，可能需要较长时间，请耐心等待.......')
for mword in wordlist:
    if not getwordcodeinfo(mword,tmp_dict):
        noinfo.append(mword+'\n')
        #print(mword+'不在json文件里！\n')
print('总共有'+str(len(noinfo))+'个词语测试失败！具体词语已保存到nointext.txt文件!')
with open(path4+'\\'+'nointext.txt',"w",encoding="utf-8") as tf:
            tf.writelines(noinfo)
#测试一词多义
#print(getwordcodeinfo('弹',tmp_dict))
